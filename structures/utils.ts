...
export function copy(
  ...
  if (((totalOffset1 - totalOffset2) & 3) !== 0 || lower4Bound >= upper4Bound) {
    ...
    var src_32 = new Uint32Array(
      src.buffer,
      totalOffset1 + lower4Bound,
      intermediate4Length
    );
    var dest_32 = new Uint32Array(
      dest.buffer,
...
export function xor(src1: Uint8Array, b, c?, d?, e?, f?, g?): any {
  ...
  var upper4Bound = Math.min(length & (~3 + lower4Bound), length);
  if (
    ((totalOffset1 - totalOffset2) & 3) !== 0 ||
    ((totalOffset1 - totalOffset3) & 3) !== 0 ||
    lower4Bound >= upper4Bound
  ) {
    for (var i = 0; i < length; ++i) {
    ...
    var intermediate4Length = (upper4Bound - lower4Bound) >> 2;
    var src1_32 = new Uint32Array(
      src1.buffer,
      totalOffset1 + lower4Bound,
      intermediate4Length
    );
    var src2_32 = new Uint32Array(
      src2.buffer,
      totalOffset2 + lower4Bound,
      intermediate4Length
    );
  ...
  if (g === undefined) return dest;
}

export function xorThree(
  src1: Uint8Array,
  off1: number,
  src2: Uint8Array,
  off2: number,
...
export function empty(
  ...
  if (lower4Bound >= upper4Bound) {
    for (var i = 0; i < length; ++i) {
      if (src[i + offset] != 0) return false;
    }
  } else {
    for (var i = 0; i < lower4Bound; ++i) {
      if (src[i + offset] != 0) return false;
    ...
    var src_32 = new Uint32Array(
      src.buffer,
      totalOffset + lower4Bound,
      intermediate4Length
    );
    for (var i = 0; i < intermediate4Length; ++i) {
      if (src_32[i] != 0) return false;
    }
    for (var i = upper4Bound; i < length; ++i) {
      if (src[i + offset] != 0) return false;
    }
...
export function sequenceEqual(src1: Uint8Array, b, c?, d?, e?): boolean {
  ...
  if (((totalOffset1 - totalOffset2) & 3) !== 0 || lower4Bound >= upper4Bound) {
    for (var i = 0; i < length; ++i) {
      if (src1[i + off1] != src2[i + off2]) return false;
    }
  } else {
    for (var i = 0; i < lower4Bound; ++i) {
      if (src1[i + off1] != src2[i + off2]) return false;
    ...
    for (var i = 0; i < intermediate4Length; ++i) {
      if (src1_32[i] != src2_32[i]) return false;
    }
    for (var i = upper4Bound; i < length; ++i) {
      if (src1[i + off1] != src2[i + off2]) return false;
    }
...
export function promisify<T>(
  fn: (cb: (err: Error, res?: T) => void) => void
): () => Promise<T>;
export function promisify<T, A1>(
  fn: (arg1: A1, cb: (err: Error, res?: T) => void) => void
): (arg1: A1) => Promise<T>;
export function promisify<T, A1, A2>(
  fn: (arg1: A1, arg2: A2, cb: (err: Error, res?: T) => void) => void
): (arg1: A1, arg2: A2) => Promise<T>;
export function promisify<T, A1, A2, A3>(
  fn: (arg1: A1, arg2: A2, arg3: A3, cb: (err: Error, res?: T) => void) => void
...
export function promisify<T, A1, A2, A3, A4, A5, A6>(
  fn: (
    arg1: A1,
    arg2: A2,
    arg3: A3,
...
export function promisify<T, A1, A2, A3, A4, A5, A6, A7>(
  fn: (
    arg1: A1,
    arg2: A2,
    arg3: A3,
